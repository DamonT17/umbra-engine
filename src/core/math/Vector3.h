...
namespace UmbraEngine::Core::Math {
     ...
     */
         ...
         * then multiplied by the cosine of the angle between them. For normalized vectors Dot returns 1 if they point 
         * in exactly the same direction, -1 if they point in completely opposite directions and zero if the vectors 
         * are perpendicular.
         */
         ...
         * @details Interpolates between the points a and b by the interpolant t. The parameter t is clamped to the
         * range [0, 1]. This is most commonly used to find a point some fraction of the way along a line between two
         * endpoints (e.g., to move an object gradually between those points). When t = 0 returns a. When t = 1
         * returns b. When t = 0.5 returns the point midway between a and b.
         */
         ...
         * will change the current vector. If you want to keep the current vector unchanged, use normalized variable. If
         * this vector is too small to be normalized it will be set to zero.
         */
         ...
         * @return void
         *
         * @details Normalizes normal. Normalizes tangent and makes sure it is orthogonal to normal (that is, angle
         ...
         * @return Vector3<T>
         *
         * @details This function is similar to MoveTowards except that the vector is treated as a direction rather than
         * a position. The current vector will be rotated round toward the target direction by an angle of
         * maxRadiansDelta, although it will land exactly on the target rather than overshoot. If the magnitudes of
         * current and target are different, then the magnitude of the result will be linearly interpolated during
         ...
         */
        static Vector3<T> RotateTowards(const Vector3<T>& current, const Vector3<T>& target, T maxRadiansDelta,
                                        T maxMagnitudeDelta);
         ...
         * "lerp") is that the vectors are treated as directions rather than points in space. The direction of the
         * returned vector is interpolated by the angle and its magnitude is interpolated between the magnitudes of
         * from and to. The parameter t is clamped to the range [0, 1].
         */
