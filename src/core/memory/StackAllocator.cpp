...
namespace UmbraEngine::Core::Memory {
    ...
    void* StackAllocator::Allocate(const size_t& sizeBytes, Alignment alignment) {
        ...
        const uint8_t adjustment = AlignAddressAdjustmentWithHeader(reinterpret_cast<uintptr_t>(position),
    ...
    void StackAllocator::FreeLastBlock() noexcept {
        ...
        usedBytes -=
                reinterpret_cast<uintptr_t>(position) - reinterpret_cast<uintptr_t>(prevPosition) + header->adjustment;
